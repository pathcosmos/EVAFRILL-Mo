...
EVAFRILL-Mo는 7개 주요 단계를 거친 반복적 설계 여정의 결과물입니다.
...
### 2단계 — EVAFRILL-Mo (하이브리드 진화)

NVIDIA의 [Nemotron-H](https://arxiv.org/abs/2504.03624) 논문에서 영감을 받아 **하이브리드 Mamba-2 + Transformer** 아키텍처로 전환했습니다:

- [Mamba-2](https://arxiv.org/abs/2405.21060) selective scan 직접 구현 (`model/mamba_block.py`)
- Mamba 블록에 SwiGLU FFN 추가 (Nemotron-H의 핵심 혁신, `mamba_d_ffn` config)
- 초기 40층 모델: Mamba-2 37개 + Attention 3개 레이어 (`mamba_d_ffn=4608`, 총 ~4.44B 파라미터)
- 이후 하드웨어 제약과 [Chinchilla 스케일링 법칙](https://arxiv.org/abs/2203.15556)에 따라 3B (26층) 규모로 최적화
...
- **완료 예상**: 2026-03-09 (월) ~17:00

### 현재 학습 상태 (2026-03-07 01:51 기준)

```
step   2,020 / 319,772 (0.63%)
loss   4.73  (정상 감소 중)
lr     9.48e-05 (워밍업 진행 중, 목표 3e-4)
tok/s  36,320 (per-GPU, 안정)
mem    47.3 GB/GPU (안정)
gnorm  0.891 (안정)
```
