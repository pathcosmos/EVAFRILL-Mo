...
**향후 개선 방향:**
1. **Repetition penalty 디코딩** — 추론 시 rep_penalty=1.2 적용이 가장 즉각적인 효과
2. **더 큰 규모의 preference 데이터** — 반복 vs 비반복 쌍을 명시적으로 포함
3. **ORPO 재시도** — SFT부터 다시 시작하여 학습 단계에서 선호도를 동시에 학습
4. **다중 α 실험** — α=0.3(SFT 70% + DPO 30%)으로 SFT 지식 비중 높이기
